...
- [GitLab example](examples/gitlab)
...
| <a name="input_atlantis_repo_config"></a> [atlantis\_repo\_config](#input\_atlantis\_repo\_config) | Atlantis repo config. If will be provided as `repo_config_json` if `repo_config_json` is not explicitly provided | <pre>object({<br>    repos = optional(list(object({<br>      id                     = string<br>      allowed_overrides      = optional(list(string), [])<br>      allow_custom_workflows = optional(bool, false)<br>    })), [])<br>  })</pre> | `{}` | no |
| <a name="input_atlantis_server_config"></a> [atlantis\_server\_config](#input\_atlantis\_server\_config) | Atlantis server config. If any option is not available here, it can be passed by `environment_variables` variable | <pre>object({<br>    repo_config_json = optional(string)<br>    repo_allowlist   = optional(string)<br>  })</pre> | `{}` | no |
...
| Name | Source | Version |
|------|--------|---------|
| <a name="module_azure_container_group"></a> [azure\_container\_group](#module\_azure\_container\_group) | getindata/container-group/azurerm | 1.0.0 |
